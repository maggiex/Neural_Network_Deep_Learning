...
class Softmax(object):
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    a -= np.max(a, axis = 0, keepdims = True) #shift a so that the highest number is 0, max for every sample
    #print(np.max(a, axis = 0, keepdims = True).shape) #(10,1)    
    
    #sum_a = np.sum(np.exp(a), axis = 0, keepdims = True) #(10,1)
    softmaxes = np.exp(a) / np.sum(np.exp(a), axis = 0, keepdims = True)  #(10,500) is this broadcasting too?
    
    
    loss = np.sum(-np.log(softmaxes[y, np.arange(num_train)]))
    
    grad_scores = np.zeros(softmaxes.shape) #np.zeros_like  
    grad_scores[y, np.arange(num_train)] = 1 #indicator function gives 1 if y = j, otherwise 0 #(10,500）
